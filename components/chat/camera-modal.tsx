      ...
      if (context) {
        ...
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            // Создаем уникальное имя файла с временем и случайным идентификатором
            const timestamp = Date.now();
            const randomId = Math.floor(Math.random() * 10000); // Добавляем случайное число для уникальности
            const newImage: FileData = {
              name: `camera_photo_${timestamp}_${randomId}.jpg`,
              size: blob.size,
              type: 'image/jpeg',
              url: url
            };
          ...
          setTimeout(() => setIsCapturing(false), 300); // Delay to match the flash animation
        }, 'image/jpeg', 0.9);
      }
    }
  };
