        ...
        {images.length === 1 ? (
          ...
          >
            <img 
              ...
              onError={(e) => {
                console.error("Failed to load image:", images[0].url);
                const target = e.target as HTMLImageElement;
                target.onerror = null;
                
                // Try to load from cache
                import('../../lib/utils').then(async ({ cacheImageData }) => {
                  const cachedData = await cacheImageData(images[0].url);
                  if (cachedData) {
                    // Update the cached_data property for future use
                    images[0].cached_data = cachedData;
                    target.src = cachedData;
                  } else {
                    // Fall back to placeholder if caching fails
                    target.src = "/file-icons/image.svg";
                  }
                }).catch(() => {
                  // If module import fails, use fallback immediately
                  target.src = "/file-icons/image.svg";
                }).finally(() => {
                  // Count as loaded regardless
                  handleImageLoad();
                });
              }}
            />
              ...
              >
                <img 
                  ...
                  onError={(e) => {
                    ...
                    // Try to load from cache
                    import('../../lib/utils').then(async ({ cacheImageData }) => {
                      const cachedData = await cacheImageData(image.url);
                      if (cachedData) {
                        // Update the cached_data property for future use
                        image.cached_data = cachedData;
                        target.src = cachedData;
                      } else {
                        // Fall back to placeholder if caching fails
                        target.src = "/file-icons/image.svg";
